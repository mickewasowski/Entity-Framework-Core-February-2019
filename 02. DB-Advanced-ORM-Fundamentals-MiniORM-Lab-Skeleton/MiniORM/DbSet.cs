...
namespace MiniORM
{
        ...
        where TEntity : class, new()
    {
        ...
        public bool Remove(TEntity item)
        {
            if (item == null)
            {
                throw new ArgumentNullException(nameof(item), "item cannot be null!"); //todo Item ???
            }
        ...
        IEnumerator IEnumerable.GetEnumerator() // todo IEnumerator first???
        {
            return this.GetEnumerator();
        }

        private void RemoveRange(IEnumerable<TEntity> entities)
        {
